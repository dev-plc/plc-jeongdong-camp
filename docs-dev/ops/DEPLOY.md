...
시트로 돌아가 새로고침하면 상단에 **`🧭 캠프 앱`** 와 **`🔄 동기화 관리`** 메뉴가 뜹니다.
아래 순서대로 실행하세요.

1. **`🧭 캠프 앱 → 일정 변경 (회차 날짜)`** — ⚠ **지금은 필수입니다.**
...
> **회차를 하나 더 만들려면** (사전답사 리허설 등) `🧭 캠프 앱 → 회차 추가 (사전답사 등)`.
...
### 1-6-1. v16 올리기 (거점 스태프·교역자·점수·시상) — 🔴 9/30 사전답사 **뒤에**

순서가 중요합니다. **GAS 가 먼저, 앱(`docs/`) 머지가 나중**입니다 — 앱이 먼저 올라가면 없는 액션을 부릅니다.

1. **바뀐 `.gs` 다섯 개**를 붙여넣습니다: `Auth.gs` · `Code.gs` · `Journal.gs` · `Setup.gs` · `Sheets.gs`
   (`MasterSync.gs` · `Mirror.gs` 는 그대로). 줄 수는 배포 안내에 따로 적습니다.
2. 배포 관리 → 편집 → **새 버전** → 배포. `?action=health` 의 `versions` 에서 다섯 파일이 `v16`,
   나머지 둘은 `v15` 인지 봅니다.
3. 시트 메뉴 **`초기 세팅 실행`** 을 **한 번 더** 돌립니다. 기존 값은 안 바뀝니다(D-021). 이걸로
   - `마스터` 끝에 **`담당 지점`**, `Progress` 끝에 **`점수출처`**, `Journal` 끝에 **`수상`** 열이 생기고
   - `역할` 드롭다운이 `일반/조장/스태프/교역자` 로, `담당 지점` 드롭다운(지점코드)이 걸립니다.
4. `마스터` 에서 **`사역자` → `교역자`** 로 바꿉니다(v16 부터 `사역자` 는 조장 권한이 없습니다).
5. 교역자·스태프 행: `조 배정` 은 비우고, 거점 스태프만 `담당 지점` 에 `CP1`~`CP4`.
   두 회차를 다 일하면 **`참여 일자` 도 비웁니다**(한 사람 한 행 — SHEET-SCHEMA 1절).
6. **`명단 점검`** — `역할·담당 지점`, `새 열` 블록이 ✅ 인지 봅니다.
7. 그다음 앱 머지(GitHub Pages). 스태프·교역자 한 명씩 실제로 로그인해 봅니다.

### 1-6-2. v17 올리기 (새 캠프 시작·Config 문구·슬라이드쇼) — 🔴 본 캠프(11/07) **뒤에**

캠프 중에는 데이터를 지우는 메뉴가 시트에 있을 필요가 없어서 따로 둡니다(D-053).

1. **바뀐 `.gs` 네 개**: `Code.gs` · `MasterSync.gs` · `Setup.gs` · `Sheets.gs` (`Auth.gs` · `Journal.gs` 는 v16, `Mirror.gs` 는 v15 그대로).
2. 새 버전 → 배포. `health` 의 `versions` 에서 네 파일이 `v17`.
3. **`초기 세팅 실행`** 을 한 번 더 — `Config` 에 `CAMP_TAGLINE` · `AUDIENCES` 가 생깁니다(기존 값은 그대로).
   시트 메뉴 이름이 `🧭 정동캠프` → **`🧭 캠프 앱`** 으로 바뀝니다(새로고침).
4. 그다음 앱 머지. 새 캠프 준비는 [운영자 가이드](OPERATOR-GUIDE.md#다음-캠프).
...
6. 메뉴 `🧭 캠프 앱 → 미러 지금 갱신` 으로 한 번 밀어 보고,
...
> 공유 버전은 **최신(Latest)** 으로 설정돼 있습니다(2026-09-26). 다시 올리면 링크를
> 이미 받은 분들에게도 **바로** 새 화면이 보입니다.
> 누가 공유 메뉴에서 **특정 버전을 고르면** 그 뒤로는 링크가 따라오지 않습니다 —
> 9/1 부터 3주간 그랬습니다. 그때는 **공유(Share) → 공유 버전 → 최신** 으로 되돌리세요.
> 🔴 확인은 **시크릿 창이나 다른 사람 폰**으로 합니다. 소유자 본인이 열면 늘 최신이 보여
> 고정돼 있어도 알아챌 수 없습니다.
...
- [ ] 테스트 참가자로 로그인되는지 — 이름 뒤 구분번호를 빼고 입력해도 되는지
      (로그인 화면에 날짜 선택은 없습니다 — 명단의 `참여 일자` 로 들어갑니다, D-027)
- [ ] 홈 맨 위 카드가 맞는지 — 캠프 전이면 `D-N`, 당일이면 **지금 하는 일정**과 코스 진행 (D-050)
- [ ] 조장 계정으로 코스 화면에서 `도착했어요` → `완료했어요` 가 눌리고 시트에 기록되는지.
      `완료 취소`·`도착 취소` 는 확인창을 거치는지
- [ ] 운영 콘솔 **진행** 탭이 폰에서 조별 카드로 보이고, 지점 네 칸이 화면 안에 들어오는지
...
- **답사 중 모니터링**: 운영 콘솔 **진행** 탭. 완주 전인데 **40분 넘게 새 기록이 없는 조**에
  ⚠ 가 붙고 맨 위에 개수가 뜹니다 — 조장에게 먼저 연락해 보세요 (D-050).
  기록을 잊은 경우가 대부분이고, 정말 늦는 조면 다음 지점을 건너뛰게 할지 정합니다.
- **거점 스태프** (v16): 로그인하면 코스 탭이 **내 지점**입니다. 이 지점에 오는 순서대로 조가 보이고
  "도착 확인 → 완료", 퀴즈 점수를 넣습니다. **스태프가 넣은 점수가 우선** — 조장은 그 점수를 못 바꿉니다.
- **교역자** (v16): 코스 탭이 **진행**(읽기 전용). 정정·검수·공지는 운영 콘솔(PIN)에서.
- **잘못 찍힌 기록**: 운영 콘솔 **진행** 탭에서 칸을 눌러 고칩니다(v16). 시트를 직접 고치면 사본에 안 가서
  조장 화면이 옛 값을 보입니다 — 콘솔에서 고치면 사본까지 밀립니다.
- **마무리 카페**: 일지가 몰리면 **일지 검수 → 📷 사진 있는 글만 → "보이는 대기 N건 모두 승인"** (v16).
- **시상(주일)**: 운영 콘솔 **🏆 시상** (v16) — 퀴즈 합계 · 소요 시간(네 곳 모두 마친 조만) · 사진(★ 지정).
  "조장·관리자 입력 포함 — 확인" 이 붙은 조는 점수를 한 번 확인하세요. 발표는 **크게 보기**.
